...
public class Er7MessageValidatorImpl implements Er7MessageValidator {
	...
	public String validatetoJson(String title, String er7Message,
		...
		try {
			...
			ConformanceContext c = DefaultConformanceContext.apply(
					IOUtils.toInputStream(constraintsXml)).get();
			...
			Map<String, Function3<Plugin, Element, Separators, EvalResult>> pluginMap = Map$.MODULE$
					.empty();
			ValueSetLibrary valueSetLibrary = valueSets != null ? ValueSetLibrary
					.apply(IOUtils.toInputStream(valueSets)).get() : null;
			SyncHL7Validator validator = new SyncHL7Validator(profile,
					valueSetLibrary, c, pluginMap);
