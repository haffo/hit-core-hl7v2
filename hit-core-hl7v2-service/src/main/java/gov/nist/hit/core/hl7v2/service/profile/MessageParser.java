...
public class MessageParser {
	...
	public ProfileModel parse(Message message, String constraintsXml)
	...
	private Set<Constraint> findConfStatements(String type, String id,
		...
		if (id != null)
			constraints.addAll(constraintManager.findConfStatementsByIdAndPath(
					type, id, constraintPath));
		if (name != null)
			constraints
					.addAll(constraintManager.findConfStatementsByNameAndPath(
							type, name, constraintPath));
		return constraints;
	}
	...
	private Set<Predicate> findPredicates(String type, String id, String name,
		...
		if (id != null)
			constraints.addAll(constraintManager.findPredicatesByIdAndTarget(
					type, id, constraintPath));
		if (name != null)
			constraints.addAll(constraintManager.findPredicatesByNameAndTarget(
					type, name, constraintPath));
		return constraints;
	}
	...
	private ProfileElement process(SegmentRef ref, Req req,
		...
		element.setReference(new gov.nist.hit.core.domain.SegmentRef(
	...
	private boolean relevent(ProfileElement child, ProfileElement parent) {
		if (parent != null) {
			return relevent(child.getUsage())
					&& relevent(parent, parent.getParent());
		} else {
			return relevent(child.getUsage());
		}
	}
	...
	private boolean relevent(String usage) {
		return usage == null || usage.equals("R") || usage.equals("RE")
				|| usage.equals("C") || usage.startsWith("C");
	}
	...
	private void setSubComponentTypes(ProfileElement subComponent) {
		...
		if (subComponent.getChildren() != null
				&& !subComponent.getChildren().isEmpty()) {
	...
	private ProfileElement process(Component c, Datatype d,
		...
		element.setType(parent.getType().equals(TYPE_FIELD) ? TYPE_COMPONENT
				: TYPE_SUBCOMPONENT);
		element.setIcon(parent.getType().equals(TYPE_FIELD) ? ICON_COMPONENT
				: ICON_SUBCOMPONENT);
		...
		element.getPredicates()
				.addAll(findPredicates(NODE_DATATYPE, d.id(), d.name(),
						constraintPath));
		parent.getChildren().add(element);
