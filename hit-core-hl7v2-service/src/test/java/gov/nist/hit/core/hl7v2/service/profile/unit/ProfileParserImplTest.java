...
public class ProfileParserImplTest {
	...
	public void testParse() throws ProfileParserException, IOException {
		String profile = IOUtils.toString(ProfileParserImplTest.class
				.getResourceAsStream("/IZ_V1.5_ACK_Z23/Profile.xml"));
		String consStatements = IOUtils
				.toString(ProfileParserImplTest.class
						.getResourceAsStream("/IZ_V1.5_ACK_Z23/ConformanceStatementConstraints.xml"));
		String predicates = IOUtils
				.toString(ProfileParserImplTest.class
	...
	private void printConstraints(ProfileElement element) {
		if (element.getConformanceStatements() != null
			...
			System.out.println("--------------->Conformance Statements:"
					+ element.getPath() + "<-------------------");
			for (Constraint co : element.getConformanceStatements()) {
				System.out.println("Conformance Statement: " + co.getId()
						+ "--" + co.getDescription());
			}
		}

		if (element.getPredicates() != null
				&& !element.getPredicates().isEmpty()) {
			System.out.println("--------------->Predicates:"
					+ element.getPath() + "<-------------------");
			for (Constraint pre : element.getPredicates()) {
				System.out.println("Predicate: " + pre.getId() + "--"
						+ pre.getDescription());
			}
		}
