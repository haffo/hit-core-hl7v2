...
public class SoapValidationResult extends ValidationResult {
	...
	public SoapValidationResult( gov.nist.healthcare.core.validation.soap.SoapValidationResult result,
		...
		if (result != null) {
			...
			Iterator<gov.nist.healthcare.core.validation.message.v3.MessageFailureV3> it = result
					.getErrors();
			while (it.hasNext()) {
	...
	private String generateXml(String title, String xml) {
		try {
			HL7V3MessageValidationReportDocument report = HL7V3MessageValidationReportDocument.Factory
					.parse(xml);
			...
			ReportHeader header = report.getHL7V3MessageValidationReport()
					.getHeaderReport();
	...
	public ValidationResultItem getValidationResultItem(MessageFailureV3 failure) {
		...
		return new ValidationResultItem( description,   column,  line,
				  path,   failureSeverity,   elementContent,
				  assertionDeclaration,   userComment,
				  assertionResult,   failureType);
	}
