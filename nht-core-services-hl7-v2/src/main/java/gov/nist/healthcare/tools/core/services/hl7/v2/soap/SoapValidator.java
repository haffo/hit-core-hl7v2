 ...
 * SoapValidator.java July 19, 2011
 *
...
public class SoapValidator {
     ...
     */
    public ArrayList<MessageFailureV3> validate(XmlObject soapMessage,
            File schematron, String phase) throws XmlException {
        ArrayList<MessageFailureV3> result = new ArrayList<MessageFailureV3>();
        ArrayList<MessageFailureV3> schemaFailures = validateWithSchema(soapMessage);
        List<MessageFailureV3> schematronFailures = validateWithSchematron(
                soapMessage, schematron, phase);        
    ...
    public ArrayList<MessageFailureV3> validate(XmlObject soapMessage,
            InputStream schematron,String phase) throws XmlException {
        ArrayList<MessageFailureV3> result = new ArrayList<MessageFailureV3>();
        ArrayList<MessageFailureV3> schemaFailures = validateWithSchema(soapMessage);
        List<MessageFailureV3> schematronFailures = validateWithSchematron(
                soapMessage, schematron, phase);
    ...
    private ArrayList<MessageFailureV3> validateWithSchema(XmlObject myDoc) {
        ...
        if (cur.toChild(ns, "Header")) {
            failures.addAll(checkElement(cur.getObject(),
                    AssertionTypeV3Constants.WSA));
        }
        return failures;
    }
    ...
    private List<MessageFailureV3> validateWithSchematron(XmlObject xml,
            File schematronFile, String phase) throws XmlException {
        StringBuilder result = new StringBuilder();
         StreamSource schematron = new StreamSource(schematronFile);
         StreamSource skeleton = new StreamSource(
                SoapValidator.class.getClassLoader().getResourceAsStream(
                        MessageValidationConstants.XSLT_SKELETON));
         //MDI:  I think this next line converts a "phase" section in the Schematron file to XSLT, with the results going into 'schematronTransform'
    ...
    private List<MessageFailureV3> validateWithSchematron(XmlObject xml,
            InputStream schematronFile, String phase) throws XmlException {
        StringBuilder result = new StringBuilder();
        StreamSource schematron = new StreamSource(schematronFile);
        StreamSource skeleton = new StreamSource(
                SoapValidator.class.getClassLoader().getResourceAsStream(
    ...
    private Node doTransform(StreamSource xmlSource, StreamSource xsltSource,
        ...
        try {
            ...
            mytransformer.transform(new DOMSource(result.getNode()),
